...
function renderRecLanes() {
  ...
  _recTracks.forEach((rt, idx) => {
    ...
    const autoLabel = isAudio ? tr('studio.p.audioN', { n: ++audN }) : tr('studio.p.recN', { n: ++recN });
...
function pmStart() {
  ...
  const btn = $('pm-playstop'); if (btn) { btn.classList.add('on'); btn.textContent = tr('studio.pm.stop'); }
}
function pmStop() {
  if (!_pmPlaying) return;
  _pmPlaying = false;
  clearInterval(_pmTimer); _pmTimer = null;
  const btn = $('pm-playstop'); if (btn) { btn.classList.remove('on'); btn.textContent = tr('studio.pm.start'); }
}
function pmSetBpm(v) {
...
function wire() {
  ...
  // 연습용 메트로놈 — 초기값을 저장해 둔 값으로 맞추고, 조작을 이벤트에 건다
  ...
  const selectTool = (name) => {
    ...
    if (name !== 'metro-practice') pmStop();   // 다른 도구로 넘어가면 계속 안 울리게
  };
