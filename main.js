...
// ── STT(자동 자막) — Whisper-small(ONNX, 양자화) on-demand 다운로드 ──────
// stem 모델(MODELS)과 같은 패턴(userData 에 캐시, on-demand, 진행률 이벤트)이지만
// "하나의 파일"이 아니라 "인코더+디코더 ONNX 두 개 + 토크나이저/설정 여러 개" 세트를
// 통째로 받아야 실제 추론이 가능해서 별도 레지스트리로 뺐다. GitHub Release 'stt-v1'
// (whalemindbass/yt-separator-releases, htdemucs 와 같은 계정/리포)에서 받는다.
const STT_RELEASE_BASE = 'https://github.com/whalemindbass/yt-separator-releases/releases/download/stt-v1/';
const STT_FILES = [
  { name: 'encoder_model_quantized.onnx', size: 92324809 },
  { name: 'decoder_model_merged_quantized.onnx', size: 156780950 },
  { name: 'tokenizer.json', size: 2480466 },
  { name: 'vocab.json', size: 1036584 },
  { name: 'merges.txt', size: 493869 },
  { name: 'normalizer.json', size: 52666 },
  { name: 'tokenizer_config.json', size: 282683 },
  { name: 'added_tokens.json', size: 2082 },
  { name: 'special_tokens_map.json', size: 2194 },
  { name: 'generation_config.json', size: 3837 },
  { name: 'preprocessor_config.json', size: 339 },
  { name: 'config.json', size: 2232 },
];
const STT_TOTAL_SIZE = STT_FILES.reduce((s, f) => s + f.size, 0);
function sttDir() {
  const dir = path.join(app.getPath('userData'), 'stt-model');
  fs.mkdirSync(dir, { recursive: true });
  return dir;
}
function sttFilePath(name) { return path.join(sttDir(), name); }
function sttInstalled() { return STT_FILES.every((f) => fs.existsSync(sttFilePath(f.name))); }
function sendSttProgress(payload) {
  if (mainWindow?.webContents && !mainWindow.webContents.isDestroyed()) {
    mainWindow.webContents.send('stt:modelDownloadProgress', payload);
  }
}
// 하나의 파일을 리다이렉트 따라가며 받는다 — stem 모델 다운로드(ensureModel)와 같은
// 방식이지만, 진행률 콜백을 넘겨받아 "여러 파일 합쳐 하나의 진행률"로 보고할 수 있게 뺐다.
function downloadOneFile(url, dest, expectedSize, onBytes) {
  return new Promise((resolve, reject) => {
    const tmp = dest + '.part';
    try { fs.unlinkSync(tmp); } catch {}
    const fetchOnce = (u, redirects = 0) => {
      if (redirects > 5) return reject(new Error('too many redirects'));
      const https = require('https');
      const req = https.get(u, { headers: { 'User-Agent': 'yt-separator' } }, (res) => {
        if ([301, 302, 303, 307, 308].includes(res.statusCode)) { res.resume(); return fetchOnce(res.headers.location, redirects + 1); }
        if (res.statusCode !== 200) return reject(new Error(`HTTP ${res.statusCode}: ${path.basename(dest)}`));
        const out = fs.createWriteStream(tmp);
...
let _sttDownloading = false, _sttCancel = false;
async function ensureSttModel() {
  if (sttInstalled()) return true;
  if (_sttDownloading) throw new Error('이미 다운로드 중');
  _sttDownloading = true; _sttCancel = false;
  let done = 0;
  sendSttProgress({ phase: 'start', total: STT_TOTAL_SIZE });
  try {
    for (const f of STT_FILES) {
      if (_sttCancel) throw new Error('취소됨');
      const dest = sttFilePath(f.name);
      if (fs.existsSync(dest)) { done += f.size; sendSttProgress({ phase: 'progress', received: done, total: STT_TOTAL_SIZE }); continue; }
      await downloadOneFile(STT_RELEASE_BASE + f.name, dest, f.size, (n) => {
        done += n; sendSttProgress({ phase: 'progress', received: done, total: STT_TOTAL_SIZE });
      });
    }
    sendSttProgress({ phase: 'done' });
    return true;
  } finally { _sttDownloading = false; }
}
ipcMain.handle('stt:status', () => ({ installed: sttInstalled(), downloading: _sttDownloading, totalSize: STT_TOTAL_SIZE }));
ipcMain.handle('stt:ensureModel', async () => {
  try { await ensureSttModel(); return { ok: true }; }
  catch (e) { return { ok: false, error: e.message }; }
});
ipcMain.handle('stt:cancelDownload', () => { _sttCancel = true; return { ok: true }; });
// 렌더러(워커)에게 encoder/decoder ONNX 바이트 + 토크나이저/설정 텍스트를 전달.
ipcMain.handle('stt:modelBytes', () => {
  if (!sttInstalled()) return { ok: false, error: 'not installed' };
  const readBin = (name) => { const b = fs.readFileSync(sttFilePath(name)); return b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength); };
  const readText = (name) => fs.readFileSync(sttFilePath(name), 'utf-8');
  return {
    ok: true,
    encoder: readBin('encoder_model_quantized.onnx'),
    decoder: readBin('decoder_model_merged_quantized.onnx'),
    vocab: readText('vocab.json'),
    addedTokens: readText('added_tokens.json'),
    specialTokens: readText('special_tokens_map.json'),
    generationConfig: readText('generation_config.json'),
    config: readText('config.json'),
  };
});
// 클립의 소스 구간([start,end), 절대 시각)을 Whisper 가 기대하는 형식(16kHz 모노 f32 PCM)
// 으로 뽑는다 — stem:extractAudio(44.1kHz 스테레오, 분리용)와는 용도가 달라 새로 둔다.
ipcMain.handle('stt:extractAudio16k', async (_ev, file, start, end) => {
  if (typeof file !== 'string' || !fs.existsSync(file)) return { ok: false, error: '파일이 존재하지 않음' };
  const tmp = path.join(app.getPath('temp'), 'yss-stt-' + crypto.randomBytes(4).toString('hex') + '.raw');
  const args = ['-nostdin', '-hide_banner', '-loglevel', 'error',
    '-i', file, '-ss', Number(start).toFixed(6), '-to', Number(end).toFixed(6),
    '-f', 'f32le', '-acodec', 'pcm_f32le', '-ar', '16000', '-ac', '1', '-y', tmp];
  return await new Promise((resolve) => {
    let proc; try { proc = spawn(FFMPEG_BIN, args, { windowsHide: true }); }
    catch (e) { return resolve({ ok: false, error: String(e.message || e) }); }
    let stderr = '';
    ...
    proc.on('error', (e) => resolve({ ok: false, error: String(e.message || e) }));
    proc.on('close', (code) => {
      if (code !== 0) { try { fs.unlinkSync(tmp); } catch {} return resolve({ ok: false, error: 'ffmpeg exit ' + code + ': ' + stderr.slice(-300) }); }
      try {
        const buf = fs.readFileSync(tmp);
        fs.unlinkSync(tmp);
        resolve({ ok: true, sampleRate: 16000, pcm: buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength) });
