...
(async () => {
  ...
  if (fs.existsSync(OUT)) {
    ...
    // t=1.8초(끝 무렵, 키프레임 9번째와 정확히 일치): 우하단 쪽에 노란색, 좌상단엔 없어야 함.
    // 상자 중앙(0.635*320+24≈227, 0.635*240+24≈176) 근처를 찍는다 — 예전엔 세그먼트를
    // midpoint(=1.9초 시점 위치)로 고정해서 찍었던 좌표(250,190, 실제로는 상자가 그 시점보다
    // 더 나아간 자리)를 그대로 썼는데, 이제 세그먼트 경계에서도 연속적으로 정확한 실제
    // 위치(1.8초 그 자체의 상자 자리)를 그리니 상자 가장자리에 걸쳐 있던 그 좌표는 프레임
    // 반올림에 따라 상자 안팎을 오갈 수 있어(정확도가 오히려 더 좋아진 결과) 불안정하다 —
    // 가장자리 대신 상자 중앙을 찍어야 튼튼하다.
    const end = samplePixel(OUT, 1.8, 227, 176);
